  ...
  if (not line.strip().startswith('!')):
    ...
    if (line.startswith('runtype')):
      ...
      if ('modified_BVF_Ri_limiter' in parameters):
        if (not any('l_limiter_setup_for_cnvg_test' in tmpstr for tmpstr in input_lines)):
          lnew = 'l_limiter_setup_for_cnvg_test = .false.,' + '\n'
          line_collections.append(lnew)
          print('append line '+lnew)
...
if ('modified_BVF_Ri_limiter' in parameters):
  config_strings['l_limiter_setup_for_cnvg_test'] = '.true.,'
