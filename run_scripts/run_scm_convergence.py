...
#
# Description:
# Script to configure and run convergence tests for BOMEX, RICO, DYCOMS2_RF02 
# and Wangara cases 
...
import sys
import shutil
from function_convergence import construct_ic_profile
...
parser.add_argument('-default-ic', action='store_true',
  help='use method in default case setup (space dependent) instead of fixed height')
parser.add_argument('-default-bc', action='store_true',
  help='use method in default case setup (linear interpolation) instead of cubic-spline interpolation')
parser.add_argument('-default-fc', dest='default_forcing', action='store_true',
  help='use method in default case setup (time-dependent forcing) instead of non time-dependent forcing')
parser.add_argument('-default-spl', dest='default_splat', action='store_true',
...
parser.add_argument('-default-tau', action='store_true',
  help='use formula of tau (for wpxp) in default case setup without smoothed Heavidise function')
parser.add_argument('-default-df', dest='default_diffusion', action='store_true',
  help='use nonlinear diffusion in default case setup instead of linear diffusion')
parser.add_argument('-default-lm', dest='default_limiter', action='store_true',
  help='use limiters on BVF and Ri in default case setup instead of modified formulas')
parser.add_argument('-default-wp3cl', dest='default_wp3_skewness_clipping', action='store_true',
  help='use method in default case setup for skewness clippings on wp3 instead of a smoothed formula') 
parser.add_argument('-skip-check', action='store_true',
  help='skip the pause in the script that allows user to inspect configuration changes')
parser.add_argument('-warm-run', dest='restart_run', action='store_true',
...
for line in input_lines:
  if (not line.strip().startswith('!')):
    ...
    if (line.startswith('tridiag_solve_method')):
      if (not any('l_modify_ic_for_cnvg_test' in tmpstr for tmpstr in input_lines)):
        lnew = 'l_modify_ic_for_cnvg_test = .false.,' + '\n'
...
if ('default_ic' not in parameters):
  config_strings['l_modify_ic_for_cnvg_test'] = '.true.,'
...
if ('default_diffusion' not in parameters):
...
if ('default_limiter' not in parameters):
  config_strings['l_use_modify_limiters'] = '.true.,'

if ('default_wp3_skewness_clipping' not in parameters): 
